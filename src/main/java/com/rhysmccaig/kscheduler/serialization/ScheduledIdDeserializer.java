...
public class ScheduledIdDeserializer implements Deserializer<ScheduledId> {
  ...
  private static final ThreadLocal<ByteBuffer> TL_BUFFER = 
      ThreadLocal.withInitial(() -> ByteBuffer.allocate(ScheduledIdSerializer.MAX_SERIALIZED_SIZE));
  ...
  /**
   * Attempt to deserialize bytes into a ScheduledId.
   */
  public ScheduledId deserialize(String topic, byte[] bytes) {
    if (bytes == null) {
      return null;
    } else if (bytes.length != ScheduledIdSerializer.MIN_SERIALIZED_SIZE 
        && bytes.length != ScheduledIdSerializer.MAX_SERIALIZED_SIZE) {
